...
#each username followed by description. If no description write nil
twitter_data.each do |person|
  puts person.keys[0] + ": "
  person.each do |username, values|
    if values["description"].nil?
      puts "Description not available."
    else
      puts values["description"]
    end
  end
end
puts
#each username with total number of followers
twitter_data.each do |person|
  puts person.keys[0] + ": "
  person.each do |key, value|
    puts value["number of followers"]
  end
end
puts
#each username with latest tweet length
twitter_data.each do |person|
  person.each do |username, values|
    puts "#{person.keys[0]}'s latest tweet is #{values["latest tweet"].length} characters long."
  end
end
puts
#username followed by char count of last twenty tweets
twitter_data.each do |user_info|
  sum=0
  user_info.each do |username, info|
    info["last twenty tweets"].each do |tweet|
      sum += tweet.length
    end
    puts "#{user_info.keys.first} used #{sum} characters in their last twenty tweets."
end
end
puts
...
puts "#{most_popular} has #{highest} followers."
puts
...
twitter_data.each do |user_info|
  user_info.each do |username, info|
    if info["number of friends"] > highest
      highest = info["number of friends"]
      most_friends = username
    end
  end
end
puts "#{most_friends} has #{highest} friends."
puts
#which user has the most tweets
most_tweets=nil
highest=0
twitter_data.each do |user_info|
  user_info.each do |username, info|
    if info["number of tweets"] > highest
      highest = info["number of tweets"]
      most_tweets = username
    end
  end
end
puts "#{most_tweets} has #{highest} tweets."
puts
#which users do not have a description
users_with_description=[]
twitter_data.each do |user_info|
  user_info.each do |username, values|
    if values["description"].nil?
      puts "#{username} does not have a description."
    end
  end
end
puts
#which users have a location listed and which do not?
puts "Users who do not have a location listed:"
users_with_location=[]
twitter_data.each do |user_info|
  user_info.each do |username, values|
...
puts
puts "Users who have a location listed:"
twitter_data.each do |user_info|
  user_info.each do |username, values|
    if values["location"]
      puts username.inspect
